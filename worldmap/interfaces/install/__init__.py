...
# Modules utilisés pour l'interfaces graphique
try:
    import pygtk
except ImportError:
...
from worldmap import VERSION
from worldmap.core.constants import WORLDMAP_PATH, CONFIG_PATH


class install():
    def quitDialog(self, widget, event=None):
        if widget == self.btn_modules:
            self.msgbox("To use 'woldmap', launch it with administrator privileges")
            exit()
        else:
    ...
    def __init__(self):
        ...
        self.fenetre.set_title("worldmap - Installation Wizard") # Titre de la fenêtre
        #self.fenetre.set_decorated(False) # Cacher les contours de la fenêtre
        self.fenetre.set_icon_from_file("%s/images/icone.png" % WORLDMAP_PATH) # Spécifier une icône
        ...
        self.img_a_propos.set_from_file("%s/images/a_propos.png" % WORLDMAP_PATH)
